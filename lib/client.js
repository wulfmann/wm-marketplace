...
const Walmart = {
  ...
  Client (settings) {
    ...
    if (!settings.consumerId || !settings.secretKey || !settings.correlationId || !settings.channelType) {
  ...
  request (req, q) {
    return new Promise((resolve, reject) => {
      ...
      const signature = this.sign(credentials.secretKey, dataToSign)
  ...
  call (req, q) {
    return this.request(req, q).then((response) => {
      return response
    })
    .catch((err) => {
      console.log(err)
